...
function mount(){
  const teams=window.LJR_V20?.standings_veteranos_35,host=document.querySelector('#view-matchcenter');if(!teams?.length||!host||document.querySelector('#jr38Simulator'))return;
  const section=document.createElement('section');section.className='jr38-panel';section.id='jr38Simulator';
  section.innerHTML='<h3>¿Cómo se movería la tabla?</h3><p>Simula un partido adicional de Veteranos 35+. Supuesto: victoria 3 puntos, empate 1; desempates por diferencia y goles a favor. La clasificación oficial conserva sus datos.</p><form class="jr38-simulation-form"><label>Local<select name="home"></select></label><label>Goles local<input type="number" name="hg" min="0" max="40" step="1" required value="0"></label><label>Visitante<select name="away"></select></label><label>Goles visitante<input type="number" name="ag" min="0" max="40" step="1" required value="0"></label><button class="primary-btn" type="submit">Simular resultado</button></form><p role="status"></p><div class="jr38-table-scroll"></div>';
  host.appendChild(section);const form=section.querySelector('form'),message=section.querySelector('[role=status]'),out=section.querySelector('.jr38-table-scroll');
