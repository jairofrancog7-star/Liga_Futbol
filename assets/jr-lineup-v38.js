...
const NS='http://www.w3.org/2000/svg',KEY='jr38-lineup-draft';
...
function mount(){
  const host=document.querySelector('#view-matchcenter');if(!host||document.querySelector('#jr38Lineup'))return;
  const section=document.createElement('section');section.className='jr38-panel';section.id='jr38Lineup';
  section.innerHTML='<h3>Tu once, sobre la cancha</h3><p>Prepara una propuesta de alineación y descárgala. Puedes guardar un borrador en este dispositivo.</p><div class="jr38-lineup-layout"><div><label for="jr38Formation">Formación</label><select id="jr38Formation"></select><div id="jr38LineupNames"></div><div class="jr38-editor-actions"><button class="ghost-btn" id="jr38SaveLineup">Guardar borrador</button><button class="primary-btn" id="jr38ExportLineup">Descargar cancha SVG</button></div><p id="jr38LineupMessage" role="status"></p></div><div id="jr38LineupPitch"></div></div>';
  host.appendChild(section);const select=section.querySelector('select'),names=section.querySelector('#jr38LineupNames'),preview=section.querySelector('#jr38LineupPitch'),msg=section.querySelector('[role=status]');
  ...
  section.querySelector('#jr38SaveLineup').onclick=()=>{try{localStorage.setItem(KEY,JSON.stringify({formation:select.value,names:inputs.map(x=>x.value)}));msg.textContent='Borrador guardado en este dispositivo.';}catch{msg.textContent='No se pudo guardar. Descarga la cancha para conservarla.';}};
  section.querySelector('#jr38ExportLineup').onclick=()=>{const url=URL.createObjectURL(new Blob([new XMLSerializer().serializeToString(preview.firstChild)],{type:'image/svg+xml;charset=utf-8'}));const a=document.createElement('a');a.href=url;a.download='liga-jr-alineacion-'+select.value+'.svg';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);msg.textContent='Cancha descargada. Es una propuesta; no modifica la alineación oficial.';};render();
}
