...
const logo=t=>data?.team_logos?.[t]?.local||TEAM_LOGOS[norm(t)]||'';
...
function sheet(c,home,away,folio=''){
 return '<article class="sheet"><div class="sheet-head"><img src="./assets/liga-logo.webp" alt="Liga Juventino Rosas"><div><small>LIGA MUNICIPAL DE FÚTBOL</small><h2>Juventino Rosas A.C.</h2><p>'+(away?'CÉDULA ARBITRAL':'PLANTILLA DE EQUIPO')+' · '+esc(c.name)+'</p></div><img src="'+({1:'./assets/categories/veteranos-50.webp',2:'./assets/categories/veteranos-35-user.png',3:'./assets/branding/primera-fuerza-hd.png',4:'./assets/categories/segunda-fuerza.webp',5:'./assets/categories/intermedia.webp'}[c.id])+'" alt="Logo categoría"><div class="folio">'+(folio?'CÉDULA #'+esc(folio):'DOCUMENTO INTERNO')+'</div></div><p class="pending">PENDIENTE DE VALIDACIÓN Y FIRMA DE LA LIGA</p><h2>'+esc(home)+(away?' vs '+esc(away):'')+'</h2><p class="meta"><span>Fecha: '+esc($('date').value||'Por confirmar')+'</span><span>Campo: '+esc($('field').value||'Por confirmar')+'</span><span>Árbitro: '+esc($('referee').value||'Por asignar')+'</span></p>'+roster(c,home)+(away?roster(c,away):'')+'<h3>Resultado final, cambios e incidencias</h3><p contenteditable="true">Jornada: ____ · Hora de inicio: ____ · Hora de término: ____ · Marcador local: ____ · Visitante: ____</p><div class="notes" contenteditable="true"></div><div class="signatures"><span>Árbitro</span><span>Delegado / capitán</span><span>Validación de la liga</span></div><small>Generada dentro de Liga Juventino Rosas con el snapshot deportivo '+esc(data.captured_at_utc||'sin fecha')+'. No redirige a una cédula de otro sitio.</small></article>';
}
