...
let DATA=null,observer=null,lastSignature='';
...
function forceHide(el){if(!el)return;el.classList.add('jr75-legacy-category-hidden');el.style.setProperty('display','none','important');el.style.setProperty('width','0','important');el.style.setProperty('height','0','important');el.style.setProperty('min-width','0','important');el.style.setProperty('max-width','0','important');el.style.setProperty('overflow','hidden','important')}
function hideLegacy(host){const cards=sourceCards(host);cards.forEach(forceHide);const rails=new Set();cards.forEach(c=>{const r=oldRailFor(c,host);if(r&&r!==host)rails.add(r)});rails.forEach(forceHide)}
...
function activate(name){const chip=$$('[data-category]').find(x=>norm(x.dataset.category)===norm(name));if(chip){chip.click();return}const src=sourceFor(name);if(src){try{src.click()}catch(_){}}try{localStorage.setItem('jrCategory',name)}catch(_){}}
function renderRow(row){const html=NAMES.map(cardMarkup).join(''),sig=html;if(lastSignature===sig&&row.dataset.jr75Rendered==='1')return;row.innerHTML=html;row.dataset.jr75Rendered='1';lastSignature=sig;$$('[data-jr75-category]',row).forEach(b=>{b.onclick=e=>{e.preventDefault();activate(b.dataset.jr75Category)}})}
function ensureRow(){const host=$('#view-more');if(!host)return false;let row=$('#jr75CategoryRow',host);if(!row){row=document.createElement('div');row.id='jr75CategoryRow';row.className='jr75-category-row';const title=$('.section-title',host),grid=$('.more-grid',host);if(title)title.insertAdjacentElement('afterend',row);else if(grid)grid.insertAdjacentElement('beforebegin',row);else host.prepend(row)}renderRow(row);hideLegacy(host);document.documentElement.dataset.jr75Fix43='ready';return true}
async function load(){try{const r=await fetch('./data/official-live.json?b='+BUILD+'&t='+Date.now(),{cache:'no-store',credentials:'omit'});if(r.ok){DATA=await r.json();lastSignature=''}}catch(_){}ensureRow()}
function schedule(){[0,40,120,300,700,1400,2600,4800,8000,12000].forEach(ms=>setTimeout(ensureRow,ms))}
function observe(){if(observer)return;observer=new MutationObserver(muts=>{if(!muts.some(m=>[...m.addedNodes].some(n=>n.nodeType===1&&!n.closest?.('#jr75CategoryRow'))))return;clearTimeout(observe.t);observe.t=setTimeout(ensureRow,60)});observer.observe(document.documentElement,{childList:true,subtree:true})}
