...
Write-Host "============================================================"
Write-Host " PUBLICAR V38 FIX13 EN GITHUB PAGES"
Write-Host "============================================================"
Write-Host ""
...
& node (Join-Path $Repo "scripts\test-v38-fix13.js") $Repo
if($LASTEXITCODE -ne 0){throw "FIX13 no paso sus pruebas. main no fue tocado."}

& git -C $Repo fetch origin --prune
if($LASTEXITCODE -ne 0){throw "git fetch fallo."}

& git -C $Repo push -u origin $Feature
if($LASTEXITCODE -ne 0){throw "No pude actualizar la rama V38. main no fue tocado."}
...
try{
  ...
  if($LASTEXITCODE -ne 0){throw "main no pudo actualizarse con fast-forward."}

  & git -C $Repo merge --no-ff $Feature -m "Publish V38 FIX13"
...
if($published){
  ...
  Write-Host ""
  Write-Host "PUBLICADO FIX13." -ForegroundColor Green
  Start-Process "https://jairofrancog7-star.github.io/Liga_Futbol/?refresh=38-13&n=$n"
}
Read-Host "Enter"
