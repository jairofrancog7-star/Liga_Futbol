...
Write-Host "============================================================"
Write-Host " PUBLICAR V38 FIX17 EN GITHUB PAGES"
Write-Host "============================================================"
Write-Host ""
Write-Host "Este paso SI modifica main." -ForegroundColor Yellow
...
$inside=(& git -C $Repo rev-parse --is-inside-work-tree 2>$null).Trim()
if($inside -ne "true"){throw "La ruta no es un repositorio Git."}
...
& node (Join-Path $Repo "scripts\test-v38-fix17.js") $Repo
if($LASTEXITCODE -ne 0){throw "FIX17 no paso pruebas. main no fue tocado."}

& git -C $Repo fetch origin --prune
if($LASTEXITCODE -ne 0){throw "fetch fallo."}

& git -C $Repo push -u origin $Feature
if($LASTEXITCODE -ne 0){throw "push de la rama V38 fallo. main no fue tocado."}
...
try{
  ...
  & git -C $Repo merge --no-ff $Feature -m "Publish V38 FIX17 Higgsfield HD diverse motion"
...
if($published){
  ...
  Write-Host ""
  Write-Host "PUBLICADO FIX17." -ForegroundColor Green
  Start-Process "https://jairofrancog7-star.github.io/Liga_Futbol/?refresh=38-17&n=$n"
}
Read-Host "Enter"
